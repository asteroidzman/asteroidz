...
static bool submit_picture(struct avk_encoder *enc, VkImage src,
	...
	/*
	 * The REFERENCE describes the PREVIOUS picture, not this one, and calling
	 * an IDR a P picture describes a DPB entry that is not what is in the
	 * slot. poc is zeroed on every key picture and incremented after each
	 * successful submit, so poc == 1 is exactly "the one before this was a
	 * key frame" -- for any IDR in the sequence, not only the first.
	 */
	...
	/* A RESET is recorded into this command buffer, so it has not happened
	 * until the buffer is submitted: latching it any earlier claims a reset
	 * that a discarded buffer never performed. */
...
bool avk_encoder_encode_frame(struct avk_encoder *enc, VkImage src,
	...
	*out_len = 0;
	...
	if (!submit_picture(enc, src, src_layout, src_x, src_y, key)) {
		 ...
		 * because the counters belong to this layer: submit_picture only
		 * reads them. */
		avk_encoder_reset_sequence(enc);
		return false;
	}
