 ...
 *
 * Layer surfaces resolve to the derived class with no rule: window rules match
 * app-id and title, and a layer surface has neither.
 */
static inline struct az_lum_rules az_lum_rules_for_surface(
	...
	LayerSurface *l = NULL;
	if (surface != NULL
			&& toplevel_from_wlr_surface(surface, &c, &l) >= 0 && c != NULL) {
		enum az_lum_class ruled;
		if (az_lum_class_from_name(c->luminance_domain, &ruled)) {
			cls = ruled;
			from_rule = true;
			r = az_lum_class_rules(cls, scene_ref_nits);
		}
		/* The Client initialises both to 1.0 and APPLY_FLOAT_PROP only
		 * overwrites above zero, so "set to something other than neutral" is
		 * the test -- an explicit 1.0 and an unset rule mean the same thing
		 * and must not stamp over a class. */
