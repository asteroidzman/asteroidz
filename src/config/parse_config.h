...
typedef struct {
	...
	int32_t hdr;				 // enable HDR (BT.2020 + PQ) output
	int32_t disable;			 // keep this output disabled
	int32_t bitdepth;			 // framebuffer bit depth (8 or 10, 0 = auto)
	float hdr_max_luminance;	 // display peak luminance (cd/m2), 0 = unset
	float hdr_min_luminance;	 // display min luminance (cd/m2), 0 = unset
	float hdr_max_fall;			 // max frame-average luminance, 0 = unset
	char *icc_profile;			 // ICC/ICM profile path (SDR mode), NULL = none
} ConfigMonitorRule;
...
typedef struct {
	...
	int32_t animation_curve_spring; // 1 = springs for geometry animations
	double spring_damping;			// < 1.0 overshoots/bounces
	double spring_frequency;		// higher = snappier settle
	...
	uint32_t shadows_contact_size;
	float shadows_contact_blur;
	int32_t shadows_contact_position_x;
	int32_t shadows_contact_position_y;
	float shadowscolor_contact[4];
	float shadows_unfocused_scale;
	float blur_unfocused_strength; // blur strength on unfocused windows
	int32_t border_gradient;	   // gradient border on the focused window
	float border_gradient_angle;
	float border_gradient_color2[4];
	/* tiled windows shrink their shadows by this factor so they don't
	 * spill across gaps onto neighbours; floating windows get the full
	 * macOS-style shadow */
	float shadows_tiled_scale;
...
int32_t parse_double_array(const char *input, double *output,
	...
	while (token != NULL && count < max_count) {
		...
		output[count] = val; // Store at the current count index
...
// Strip non-printable characters from a string (\r, \n, spaces, etc)
...
// Parse a "bind" flag combination string
void parse_bind_flags(const char *str, KeyBinding *kb) {

	// Check whether it starts with "bind"
	...
	// Iterate over the suffix characters
...
int32_t parse_circle_direction(const char *str) {
	...
	// Return the enum value matching the lowercased string
...
// Helper: check whether a string starts with the given prefix (case-insensitive)
...
uint32_t parse_mod(const char *mod_str) {
	...
	while (token != NULL) {
		...
		// Skip if the token becomes an empty string
		if (*token == '\0') {
...
// Helper function: look up the keycodes matching a keysym in the keymap
...
void create_config_keymap(void) {
	// Initialize the xkb context and keymap
...
KeySymCode parse_key(const char *key_str, bool isbindsym) {
	...
	if (sym != XKB_KEY_NoSymbol) {
		// Try to find the matching keycodes
		int32_t found_count =
			find_keycodes_for_keysym(config.keymap, sym, &kc.keycode);
		if (found_count > 0) {
			kc.type = KEY_TYPE_CODE;
			kc.keysym = sym; // Still keep the keysym for reference
		} else {
			kc.type = KEY_TYPE_SYM;
			kc.keysym = sym;
...
uint32_t parse_button(const char *str) {
	...
	lowerStr[i] = '\0'; // Ensure the string is properly terminated

	// Parse the "code:number" format
	if (strncmp(lowerStr, "code:", 5) == 0) {
		const char *numStart = lowerStr + 5; // Skip "code:"
		char *endptr;
		unsigned long val = strtoul(numStart, &endptr, 10);

		// Check the conversion succeeded with no extra characters and the value doesn't overflow (fits in uint32_t)
	...
	// Return the button number matching the lowercased string
...
FuncType parse_func_name(char *func_name, Arg *arg, char *arg_value,
	...
	if (strcmp(func_name, "focus_stack") == 0) {
		...
		/* jump is not a separate view, just the overview with keyboard
		 * hints: `toggle_overview jump` opens it in jump mode */
		if (strcmp(arg_value, "jump") == 0) {
			func = togglejump;
			(*arg).i = 0;
		} else {
			func = toggleoverview;
		...
		/* Default to cycling forward when no direction argument is given */
		...
		// Collect the parameters to concatenate
		...
		func = spawn_on_empty;
		(*arg).v = strdup(arg_value); // Note: this memory must be freed later
	...
	} else if (strcmp(func_name, "set_output_transform") == 0) {
		/* set_output_transform,<output>,<0-7> */
		func = set_output_transform;
		(*arg).v = strdup(arg_value);
		(*arg).i = atoi(arg_value2);
	} else if (strcmp(func_name, "set_output_mode_transform") == 0) {
		/* set_output_mode_transform,<output>,<WxH[@Hz]>,<0-7> */
		func = set_output_mode_transform;
		(*arg).v = strdup(arg_value);
		(*arg).v2 = strdup(arg_value2);
		...
		func = set_blur_cache;
		(*arg).i = atoi(arg_value);
	} else if (strcmp(func_name, "set_blur_cache_starve") == 0) {
		func = set_blur_cache_starve;
		(*arg).i = atoi(arg_value);
		...
		(*arg).v = strdup(arg_value);
	} else if (strcmp(func_name, "retrain_monitor") == 0) {
		func = retrain_monitor;
		(*arg).v = strdup(arg_value);
...
bool parse_option(Config *config, char *key, char *value) {
	...
	if (strcmp(key, "keymode") == 0) {
	...
	} else if (strcmp(key, "animation_curve_type") == 0) {
		config->animation_curve_spring = strcmp(value, "spring") == 0;
	} else if (strcmp(key, "spring_damping") == 0) {
		config->spring_damping = CLAMP_FLOAT(atof(value), 0.1f, 2.0f);
	} else if (strcmp(key, "spring_frequency") == 0) {
		config->spring_frequency = CLAMP_FLOAT(atof(value), 4.0f, 60.0f);
	...
	} else if (strcmp(key, "sdr_reference_luminance") == 0) {
	...
	} else if (strcmp(key, "blur_transparency_threshold") == 0) {
		...
		config->shadows_contact_size = atoi(value);
	} else if (strcmp(key, "shadows_contact_blur") == 0) {
		config->shadows_contact_blur = atof(value);
	} else if (strcmp(key, "shadows_contact_position_x") == 0) {
		config->shadows_contact_position_x = atoi(value);
	} else if (strcmp(key, "shadows_contact_position_y") == 0) {
		config->shadows_contact_position_y = atoi(value);
	} else if (strcmp(key, "shadows_unfocused_scale") == 0) {
		config->shadows_unfocused_scale = CLAMP_FLOAT(atof(value), 0.0f, 1.0f);
	} else if (strcmp(key, "shadows_tiled_scale") == 0) {
		config->shadows_tiled_scale = CLAMP_FLOAT(atof(value), 0.0f, 1.0f);
	} else if (strcmp(key, "blur_unfocused_strength") == 0) {
		config->blur_unfocused_strength = CLAMP_FLOAT(atof(value), 0.0f, 1.0f);
	} else if (strcmp(key, "border_gradient") == 0) {
		config->border_gradient = CLAMP_INT(atoi(value), 0, 1);
	} else if (strcmp(key, "border_gradient_angle") == 0) {
		config->border_gradient_angle = atof(value);
	} else if (strcmp(key, "border_gradient_color2") == 0) {
		int64_t color = parse_color(value);
		if (color == -1) {
			fprintf(stderr,
					"\033[1m\033[31m[ERROR]:\033[33m Invalid "
					"border_gradient_color2 format: %s\n",
					value);
			return false;
		} else {
			convert_hex_to_rgba(config->border_gradient_color2, color);
		}
	} else if (strcmp(key, "shadowscolor_contact") == 0) {
		int64_t color = parse_color(value);
		if (color == -1) {
			fprintf(stderr,
					"\033[1m\033[31m[ERROR]:\033[33m Invalid "
					"shadowscolor_contact format: %s\n",
					value);
			return false;
		} else {
			convert_hex_to_rgba(config->shadowscolor_contact, color);
		}
		...
		// 1. Count the commas in value to determine how many floats to parse
		int32_t count = 0; // Initialize to 0
		for (const char *p = value; *p; p++) {
			if (*p == ',')
				count++;
		}
		int32_t float_count = count + 1; // Number of floats is comma count + 1

		// 2. Dynamically allocate memory to store the floats
		...
		// 3. Parse the floats in value
		char *value_copy =
			strdup(value); // Duplicate value, since strtok modifies the original string
		...
		// 4. Check whether the number of parsed floats matches
		if (i != float_count) {
			fprintf(stderr,
					"\033[1m\033[31m[ERROR]:\033[33m Invalid "
					...
					value);
			free(value_copy);
			free(config->scroller_proportion_preset);  // Free the allocated memory
			config->scroller_proportion_preset = NULL; // Avoid a dangling pointer
			config->scroller_proportion_preset_count = 0;
			return false;
		}
		config->scroller_proportion_preset_count = float_count;

		// 5. Free the temporary copied string
		free(value_copy);
	} else if (strcmp(key, "circle_layout") == 0) {
		// 1. Count the commas in value to determine how many strings to parse
		int32_t count = 0; // Initialize to 0
		for (const char *p = value; *p; p++) {
			if (*p == ',')
				count++;
		}
		int32_t string_count = count + 1; // Number of strings is comma count + 1

		// 2. Dynamically allocate memory to store the string pointers
		...
		while (token != NULL && i < string_count) {
			...
			if (!config->circle_layout[i]) {
				...
				free(value_copy);
				config->circle_layout = NULL; // Avoid a dangling pointer
		...
		// Set default values (sentinels meaning "not set by this rule", so partial
		// rules merge cleanly with other matching rules)
		rule->name = NULL;
		rule->make = NULL;
		rule->model = NULL;
		rule->serial = NULL;
		...
		rule->disable = -1;
		rule->bitdepth = -1;
		rule->hdr_max_luminance = -1.0f;
		rule->hdr_min_luminance = -1.0f;
		rule->hdr_max_fall = -1.0f;
		rule->icc_profile = NULL;
		...
		while (token != NULL) {
			...
			if (colon != NULL) {
				*colon = '\0';
				...
				} else if (strcmp(key, "icc_profile") == 0) {
					rule->icc_profile = strdup(val);
				} else if (strcmp(key, "hdr_max_luminance") == 0) {
					rule->hdr_max_luminance =
						CLAMP_FLOAT(atof(val), 0.0f, 10000.0f);
				} else if (strcmp(key, "hdr_min_luminance") == 0) {
					rule->hdr_min_luminance =
						CLAMP_FLOAT(atof(val), 0.0f, 100.0f);
				} else if (strcmp(key, "hdr_max_fall") == 0) {
					rule->hdr_max_fall =
						CLAMP_FLOAT(atof(val), 0.0f, 10000.0f);
				} else if (strcmp(key, "bitdepth") == 0) {
					rule->bitdepth = atoi(val);
					if (rule->bitdepth != 8 && rule->bitdepth != 10) {
						fprintf(stderr,
								"\033[1m\033[31m[ERROR]:\033[33m monitor rule "
		...
		// If no layer name was specified, use the default
...
static const struct {
	...
	{"layout/scroller/edge-scroll/enable", "scroller_edge_scroll"},
	{"layout/scroller/edge-scroll/size", "scroller_edge_scroll_size"},
	{"layout/scroller/edge-scroll/delay", "scroller_edge_scroll_delay"},
	{"layout/scroller/edge-scroll/pointer-focus", "edge_scroller_pointer_focus"},
	...
	{"effects/blur/unfocused-strength", "blur_unfocused_strength"},
	{"effects/blur/passes", "blur_params_num_passes"},
	{"effects/blur/radius", "blur_params_radius"},
	{"effects/blur/params/noise", "blur_params_noise"},
	{"effects/blur/params/brightness", "blur_params_brightness"},
	{"effects/blur/params/contrast", "blur_params_contrast"},
	{"effects/blur/params/saturation", "blur_params_saturation"},
	{"effects/blur/transparency-threshold", "blur_transparency_threshold"},
	{"effects/shadow/enable", "shadows"},
	{"effects/shadow/layer", "layer_shadows"},
	...
	{"effects/shadow/unfocused-scale", "shadows_unfocused_scale"},
	{"effects/shadow/tiled-scale", "shadows_tiled_scale"},
	{"effects/shadow/contact/enable", "shadows_contact"},
	{"effects/shadow/contact/size", "shadows_contact_size"},
	{"effects/shadow/contact/blur", "shadows_contact_blur"},
	...
	{"animations/window-open/type", "animation_type_open"},
	{"animations/window-open/duration", "animation_duration_open"},
	{"animations/window-open/fade-begin-opacity", "fadein_begin_opacity"},
	...
	{"overview/hotarea/enable", "enable_hotarea"},
...
void free_circle_layout(Config *config) {
	if (config->circle_layout) {
		...
		for (int32_t i = 0; i < config->circle_layout_count; i++) {
			if (config->circle_layout[i]) {
				free(config->circle_layout[i]);	 // Free a single string
				config->circle_layout[i] = NULL; // Avoid a dangling pointer
			}
		}
		// Free the circle_layout array itself
		free(config->circle_layout);
		config->circle_layout = NULL; // Avoid a dangling pointer
	}
	config->circle_layout_count = 0; // Reset the count
}
...
void free_config(void) {
	...
	if (config.window_rules) {
		for (int32_t i = 0; i < config.window_rules_count; i++) {
			...
			rule->monitor = NULL;
			rule->special_workspace = NULL;
			// Free globalkeybinding's arg.v (if dynamically allocated)
	...
	// Clean up the keymap used for key parsing
...
void set_value_default() {
	...
	config.shadows_contact_position_x = 0;
	config.shadows_contact_position_y = 2;
	config.shadowscolor_contact[0] = 0.0f;
	config.shadowscolor_contact[1] = 0.0f;
	config.shadowscolor_contact[2] = 0.0f;
	config.shadowscolor_contact[3] = 0.30f;
	config.shadows_unfocused_scale = 0.45f;
	config.shadows_tiled_scale = 0.3f;
	config.blur_unfocused_strength = 1.0f;
	config.border_gradient = 0;
	config.border_gradient_angle = 45.0f;
	config.border_gradient_color2[0] = 1.0f;
	config.border_gradient_color2[1] = 1.0f;
	config.border_gradient_color2[2] = 1.0f;
	config.border_gradient_color2[3] = 1.0f;
...
void set_default_key_bindings(Config *config) {
	// Compute the number of default key bindings
	size_t default_key_bindings_count =
		sizeof(default_key_bindings) / sizeof(KeyBinding);

	// Reallocate memory to fit the new default key bindings
	config->key_bindings =
		realloc(config->key_bindings,
	...
	// Copy the default key bindings into the config's key bindings array
	...
	// Update the total key bindings count
	config->key_bindings_count += default_key_bindings_count;
}
...
bool parse_config(void) {
	...
	// Re-point the xkb_rules pointers at the static arrays
	...
	// Initialize the dynamic array pointers to NULL to avoid dangling pointers
	config.window_rules = NULL;
	config.window_rules_count = 0;
	config.monitor_rules = NULL;
	config.monitor_rules_count = 0;
...
void reset_blur_params(void) {
	...
	/* create or drop the per-client / per-layer blur nodes to match the
	 * reloaded config */
...
void reapply_monitor_rules(void) {
	...
	wl_list_for_each(m, &mons, link) {
		...
		if (monitor_merge_rules(m, &merged_rule)) {
			mx = merged_rule.x == INT32_MAX ? m->m.x : merged_rule.x;
			...
			m->hdr_max_luminance = merged_rule.hdr_max_luminance > 0
				? merged_rule.hdr_max_luminance : 0;
			m->hdr_min_luminance = merged_rule.hdr_min_luminance > 0
				? merged_rule.hdr_min_luminance : 0;
			m->hdr_max_fall = merged_rule.hdr_max_fall > 0
				? merged_rule.hdr_max_fall : 0;
			mon_load_icc_profile(m, merged_rule.icc_profile);

			(void)apply_rule_to_state(m, &merged_rule, &state, vrr, custom);
			if (merged_rule.disable > 0) {
				wlr_output_state_set_enabled(&state, false);
				m->asleep = 0;
			} else if (merged_rule.disable == 0 && !m->wlr_output->enabled) {
