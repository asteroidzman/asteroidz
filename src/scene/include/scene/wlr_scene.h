...
struct wlr_scene {
	...
	// SDR white level on HDR outputs in cd/m2, 0 = transfer function default
	float sdr_reference_luminance;
	// Saturation applied to SDR content on HDR outputs, 1.0 = neutral,
	// 0 = unset (treated as neutral)
...
struct wlr_scene_rect {
	...
	// Optional gradient fill (fork extension, appended for ABI compat).
	// When set, the gradient is rendered instead of the solid color; the
	// gradient range always covers the rect's box.
	bool has_gradient;
	float gradient_degree;
	int gradient_linear; // 1 = linear, 2 = conic
	int gradient_blend;
	float gradient_origin[2];
	int gradient_count;
	float *gradient_colors; // owned by the node, 4 floats per color
};
...
/**
 * Set the luminance (cd/m2) that SDR content white maps to on HDR outputs.
 * 0 restores the default (the transfer function's reference luminance,
 * 203 cd/m2 for PQ). Takes effect on the next frame.
 */
void wlr_scene_set_sdr_reference_luminance(struct wlr_scene *scene, float luminance);

/**
 * Set the saturation applied to SDR content on HDR outputs. 1.0 (or 0) is
 * neutral; >1 increases vibrancy. Applied in linear light around Rec.709
 * luma before conversion to the output's color space. Takes effect on the
 * next frame.
 */
...
/**
 * Set a gradient fill on a rectangle node (fork extension). The colors
 * array (4 floats per color, premultiplied) is copied; pass count = 0 to
 * restore the solid color fill. degree/origin/linear/blend follow
 * struct fx_gradient semantics; the range always covers the rect.
 */
...
/**
 * Change the individual corner radii of an existing shadow node.
 */
 ...
 */
/**
 * Restrict blur rendering to a pixel-accurate region (node-local
 * coordinates), e.g. a client's background-effect region including its
 * rounded corners. Pass NULL to clear.
 */
