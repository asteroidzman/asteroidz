...
static void dwindle_assign(DwindleNode *node, int32_t ax, int32_t ay,
	...
	if (!node->is_split) {
		if (node->client) {
			if (!node->client->isfullscreen &&
				...
				struct wlr_box box = {ax, ay, ASTEROIDZ_MAX(1, aw),
									  ASTEROIDZ_MAX(1, ah)};
	...
	node->container_h = ah;
	if (node->split_h) {
		int32_t w1 = ASTEROIDZ_MAX(1, (int32_t)(aw * node->ratio) - gap_h / 2);
		dwindle_assign(node->first, ax, ay, w1, ah, gap_h, gap_v);
		dwindle_assign(node->second, ax + w1 + gap_h, ay, aw - w1 - gap_h, ah,
					   gap_h, gap_v);
	} else {
		int32_t h1 = ASTEROIDZ_MAX(1, (int32_t)(ah * node->ratio) - gap_v / 2);
...
static void dwindle_resize_client(Monitor *m, Client *c) {
	...
	if (dwindle_locked_h_node) {
		float cw = (float)ASTEROIDZ_MAX(1, dwindle_locked_h_node->container_w);
	...
	if (dwindle_locked_v_node) {
		float ch = (float)ASTEROIDZ_MAX(1, dwindle_locked_v_node->container_h);
...
static void dwindle_resize_client_step(Monitor *m, Client *c, int32_t dx,
	...
	if (h_node && dx) {
		float cw = (float)ASTEROIDZ_MAX(1, h_node->container_w);
		float delta = (float)dx / cw;
		h_node->ratio = CLAMP_FLOAT(h_node->ratio + delta, 0.05f, 0.95f);
	}

	if (v_node && dy) {
		float ch = (float)ASTEROIDZ_MAX(1, v_node->container_h);
