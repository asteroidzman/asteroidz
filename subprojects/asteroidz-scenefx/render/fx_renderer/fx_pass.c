...
bool fx_render_pass_init_offscreen_buffers(struct wlr_render_pass *render_pass,
	...
	// On deep (10-bit / HDR) outputs, 8-bit intermediates would quantize
	// every blurred pixel and reintroduce banding: use half-float buffers
	// where alpha is needed and 10-bit ones elsewhere. Transfers between
	// these buffers are all shader draws, so mixed formats are fine.
	uint32_t alpha_format = 0, opaque_format = 0;
	if (output->swapchain != NULL) {
		switch (output->swapchain->format.format) {
		...
		case DRM_FORMAT_ABGR16161616:
			alpha_format = DRM_FORMAT_ABGR16161616F;
			opaque_format = DRM_FORMAT_XBGR2101010;
			break;
		default:
			break;
		}
	...
	// While an output color transform is active, render the whole frame
	// into an offscreen buffer; render_pass_submit() resolves it into the
	// real output buffer through the transform LUT
	if (pass->color_transform != NULL &&
			pass->fx_offscreen_buffers->color_transform_buffer != NULL &&
			pass->color_resolve_target == NULL) {
		pass->color_resolve_target = pass->buffer;
		pass->buffer = pass->fx_offscreen_buffers->color_transform_buffer;
	}
...
void fx_render_pass_set_color_resolve_damage(struct wlr_render_pass *render_pass,
	...
	struct fx_gles_render_pass *pass = fx_get_render_pass(render_pass);
	pixman_region32_copy(&pass->color_resolve_damage, damage);
	pass->has_color_resolve_damage = true;
}

static void pass_resolve_color_transform(struct fx_gles_render_pass *pass);
...
static bool render_pass_submit(struct wlr_render_pass *wlr_pass) {
	...
	pass_resolve_color_transform(pass);
...
out:
	...
	pixman_region32_fini(&pass->color_resolve_damage);
	if (pass->color_transform != NULL) {
...
#define FX_LUT3D_SIZE 33

static float srgb_channel_to_linear(float c) {
	return c <= 0.04045f ? c / 12.92f : powf((c + 0.055f) / 1.055f, 2.4f);
}

// Bake the output color transform into a 3D LUT indexed by electrical
// (gamma-encoded) sRGB values: perceptually uniform indexing keeps shadow
// precision, and the frame content is electrical already.
static bool fx_renderer_ensure_lut3d(struct fx_renderer *renderer,
		struct wlr_color_transform *tr) {
	if (renderer->lut3d_tex != 0 && renderer->lut3d_src == tr) {
		return true;
	}

	const int dim = FX_LUT3D_SIZE;
	float *data = malloc((size_t)dim * dim * dim * 3 * sizeof(float));
	if (data == NULL) {
		return false;
	}
	for (int b = 0; b < dim; b++) {
		for (int g = 0; g < dim; g++) {
			for (int r = 0; r < dim; r++) {
				// the transform maps linear light to output-encoded values
				float in[3] = {
					srgb_channel_to_linear((float)r / (dim - 1)),
					srgb_channel_to_linear((float)g / (dim - 1)),
					srgb_channel_to_linear((float)b / (dim - 1)),
				};
				float *out = &data[(((size_t)b * dim + g) * dim + r) * 3];
				wlr_color_transform_eval(tr, out, in);
			}
		}
	}

	if (renderer->lut3d_tex == 0) {
	...
	renderer->procs.glTexImage3DOES(GL_TEXTURE_3D_OES, 0, GL_RGB, dim, dim, dim,
			0, GL_RGB, GL_FLOAT, data);
	glBindTexture(GL_TEXTURE_3D_OES, 0);
	free(data);

	if (renderer->lut3d_src != NULL) {
		wlr_color_transform_unref(renderer->lut3d_src);
	}
	renderer->lut3d_src = wlr_color_transform_ref(tr);
	return true;
}

// Resolve the offscreen frame into the real output buffer through the
// output color transform LUT (final fullscreen pass for HDR outputs)
static void pass_resolve_color_transform(struct fx_gles_render_pass *pass) {
	if (pass->color_resolve_target == NULL) {
		return;
	}

	struct fx_renderer *renderer = pass->buffer->renderer;
	struct fx_framebuffer *src = pass->buffer;
	struct fx_framebuffer *dst = pass->color_resolve_target;
	struct color_transform_shader *shader = &renderer->shaders.color_transform;

	pass->buffer = dst;
	pass->color_resolve_target = NULL;

	// Nothing changed for this buffer age: its previous resolve is current
	if (pass->has_color_resolve_damage &&
			pixman_region32_empty(&pass->color_resolve_damage)) {
		return;
	}

	if (!renderer->exts.OES_texture_3D || shader->program == 0 ||
			!fx_renderer_ensure_lut3d(renderer, pass->color_transform)) {
		wlr_log(WLR_ERROR, "Failed to prepare the output color transform LUT");
		return;
	}
	...
	fx_framebuffer_bind(dst);
	glViewport(0, 0, dst->buffer->width, dst->buffer->height);
	glDisable(GL_BLEND);
	...
	glUniform1f(shader->lut_scale,
			(float)(FX_LUT3D_SIZE - 1) / FX_LUT3D_SIZE);
	glUniform1f(shader->lut_offset, 0.5f / FX_LUT3D_SIZE);

	struct wlr_box box = {0, 0, dst->buffer->width, dst->buffer->height};
	struct wlr_fbox src_fbox = {0, 0, 1, 1};
	set_proj_matrix(shader->proj, pass->projection_matrix, &box);
	set_tex_matrix(shader->tex_proj, WL_OUTPUT_TRANSFORM_NORMAL, &src_fbox);

	render(&box, pass->has_color_resolve_damage
			? &pass->color_resolve_damage : NULL,
		   shader->pos_attrib);
...
void fx_render_pass_add_rounded_rect_grad(struct fx_gles_render_pass *pass,
	...
	// v_color.a gates the whole shape; uniforms reset to 0 on (re)link,
	// so an explicit upload is required or the rect renders transparent
	glUniform4f(shader.color, options->color.r, options->color.g,
			options->color.b, options->color.a);
	glUniform2f(shader.size, box.width, box.height);
	glUniform2f(shader.position, box.x, box.y);
	...
	const struct wlr_box *cr_box = &fx_options->clipped_region.area;
	struct fx_corner_fradii cr_corners = fx_options->clipped_region.corners;
	glUniform2f(shader.clip_position, cr_box->x, cr_box->y);
	glUniform2f(shader.clip_size, cr_box->width, cr_box->height);
	uniform_corner_radii_set(&shader.clip_radius, &cr_corners);
	...
	apply_clip_region(&clip_region, cr_box, &cr_corners);
...
void fx_render_pass_add_box_shadow(struct fx_gles_render_pass *pass,
	...
	uniform_corner_radii_set(&renderer->shaders.box_shadow.corner_radii, &options->corners);
...
struct fx_gles_render_pass *fx_begin_buffer_pass(struct fx_framebuffer *buffer,
	...
	pixman_region32_init(&pass->color_resolve_damage);
	pass->has_color_resolve_damage = false;
	pass->has_blur = false;
