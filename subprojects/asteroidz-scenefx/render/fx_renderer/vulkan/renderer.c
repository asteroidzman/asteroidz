...
static void stage_buffer_destroy(struct fx_vk_renderer *r,
		struct fx_vk_stage_buffer *buffer) {
	if (!buffer) {
		return;
	}
...
static struct fx_vk_stage_buffer *stage_buffer_create(
		struct fx_vk_renderer *r, VkDeviceSize bsize) {
	struct fx_vk_stage_buffer *buf = calloc(1, sizeof(*buf));
	if (!buf) {
		wlr_log_errno(WLR_ERROR, "Allocation failed");
		return NULL;
	}
...
void fx_vulkan_stage_buffer_reclaim(struct fx_vk_stage_buffer *buf,
		uint64_t current_point) {
	size_t completed = 0;
	struct fx_vk_stage_watermark *mark;
	...
	if (completed > 0) {
		completed *= sizeof(struct fx_vk_stage_watermark);
...
VkDeviceSize fx_vulkan_stage_buffer_alloc(struct fx_vk_stage_buffer *buf,
...
struct fx_vk_buffer_span fx_vulkan_get_stage_span(struct fx_vk_renderer *r,
	...
	struct fx_vk_stage_buffer *buf;
	wl_list_for_each(buf, &r->stage.buffers, link) {
		VkDeviceSize offset = fx_vulkan_stage_buffer_alloc(buf, size, alignment);
	...
	struct fx_vk_stage_buffer *new_buf = stage_buffer_create(r, bsize);
	if (new_buf == NULL) {
		goto error;
	}
	...
	VkDeviceSize offset = fx_vulkan_stage_buffer_alloc(new_buf, size, alignment);
...
void fx_vulkan_stage_mark_submit(struct fx_vk_renderer *renderer,
	...
	wl_list_for_each(buf, &renderer->stage.buffers, link) {
		...
		struct fx_vk_stage_watermark *mark = wl_array_add(
		...
		*mark = (struct fx_vk_stage_watermark){
			.head = buf->head,
...
static void stage_buffer_gc(struct fx_vk_renderer *renderer, uint64_t current_point) {
	struct fx_vk_stage_buffer *buf, *buf_tmp;
	wl_list_for_each_safe(buf, buf_tmp, &renderer->stage.buffers, link) {
		if (buf->head != buf->tail) {
			buf->empty_gc_cnt = 0;
			fx_vulkan_stage_buffer_reclaim(buf, current_point);
			continue;
		}
...
bool fx_vulkan_submit_stage_wait(struct fx_vk_renderer *renderer) {
	...
	fx_vulkan_stage_mark_submit(renderer, timeline_point);
	...
	if (!fx_vulkan_wait_command_buffer(cb, renderer)) {
		return false;
	}
...
static void fx_vulkan_destroy(struct wlr_renderer *wlr_renderer) {
	...
	struct fx_vk_stage_buffer *buf, *tmp_buf;
