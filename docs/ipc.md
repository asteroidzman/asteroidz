...
| `shm_plan_promoted_full` | region plans turned into a whole-buffer copy because the damage already covered most of the buffer. A partial upload copies row by row (a rectangle's source rows are `stride` apart); a full one is a single `memcpy`, measured here at 6.1 GB/s against 1.9 GB/s. So above roughly a third coverage the precise answer is the slow one, and past the 75% threshold this copies **more bytes in less time** |
