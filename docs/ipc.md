...
| `shm_async_joins` / `shm_async_join_waits` / `shm_async_join_us_max` | how often the main thread collected a worker's copy, how often it had to **wait** for one, and the longest such wait. These separate two different outcomes that both read as "async is on": moving the copy off the event loop is worthless if the loop then blocks on the join, and `join_waits` tracking `async_jobs` one-for-one is that failure |
| `shm_worker_stolen` | copies the main thread took back because the worker was still busy with an earlier one. Climbing means one worker thread is not keeping up with the load, which is a different problem from the copy being synchronous |
| `shm_worker_pack_us_max` / `_avg` / `shm_worker_pack_mb_s` | how long the memcpy itself took, timed on the thread that performs it, and the rate that implies. Read this **against** `shm_async_join_us_max`: a long join with a short pack is a wake-up or scheduling problem, a long join with a long pack is bandwidth or page faults, and the two have no fix in common. They were once the same number — `settle()` recorded the join wait as a copy time, so `shm_copy_us_max` and `shm_async_join_us_max` were one measurement printed twice and could not disagree |
| `acquire_waits` | client acquire fences this compositor waited on before sampling a buffer. AVK previously sampled every dma-buf client with **no** producer→consumer ordering — no explicit acquire point and no implicit fence — so a frame could read a buffer while the client's GPU work was still writing it. Zero here on a desktop with dma-buf clients means the ordering is not happening |
| `acquire_explicit` / `acquire_implicit` | how those fences arrived: an acquire point stated through `wp_linux_drm_syncobj_v1`, versus an implicit fence read off the dma-buf. They are not interchangeable — a client using the protocol deliberately attaches **no** implicit fence, so checking only the dma-buf would find nothing and skip the wait silently |
| `acquire_dropped` / `acquire_import_fails` | fences discarded because more surfaces were sampled in one frame than the wait pool holds, and fences that would not import as a semaphore. **Both must be 0.** Each one is a frame submitted while a client was still drawing into a buffer it reads |
| `release_points_set` / `release_points_dropped` | release points registered against the frame's completion, and those that could not be. wlroots' default is to signal a client's release point at its *next commit*, gated by nothing — telling a client its buffer is free while the GPU is still reading it. Advertising the protocol and then doing that is worse than not advertising it, because a client stops attaching implicit fences once it sees the global |
...
| `shm_stale_from_surface` | of those, the ones served from the image the **surface** kept rather than from a live sibling buffer. Non-zero means the client allocates a fresh buffer per generation and destroys the old one, so nothing in its pool was available and only the surface-level fallback had anything to draw |
| `shm_stale_no_sibling` / `shm_stale_pool_avg` | frames that found nothing to fall back on and blocked anyway, and how many buffers were in the surface's pool when that happened. **Should be 0.** A pool of one means the client's previous buffers are already destroyed; a pool of several that still yields nothing means those siblings are unuploaded too, and the copies are simply not keeping up — different causes, which is why the pool depth is reported beside the count |
...
| `shm_stale_no_pool` | frames that blocked because the buffer was not in any surface pool, so it had no siblings to ask about at all. Distinct from the above because it is a bookkeeping failure rather than a timing one |
| `staging_reused` / `staging_reclaimed` / `staging_created` | staging allocations served **warm** from the device's cache, those served warm only after draining the retire queue on the spot, and those that had to be created and faulted. A client presenting a fresh `wl_buffer` every frame does exactly one take and one put per frame, and the put arrives through the retire queue — so a miss often means only that the queue had not been collected yet, which is what `staging_reclaimed` counts and `avk_retire_collect()` (non-blocking) recovers. A staging buffer costs one page fault per page the first time the CPU writes it, and a 56MB buffer is 13,691 of them: measured at **52ms inside the copy** (1.1GB/s) against 4ms (14GB/s) once the pages exist. A client that presents a fresh `wl_buffer` every frame — Firefox on `wl_shm` does — allocated fresh staging every frame and paid it every frame. Retired staging buffers are now kept and handed to the next allocation that fits, which is safe for exactly one reason: the retire queue only releases a buffer the GPU has finished reading. A `staging_created` that keeps climbing on a steady window means the cache is missing and the copies are paying faults |
| `upload_ring_stalls` / `upload_ring_slots` | times an upload had to wait for the command ring's next slot to be finished with by the GPU, and how deep that ring is. The wait is paid **on the frame thread**, and its length is set by how far behind the GPU is — measured at 5–34ms each, 61 times in 52 seconds, on a 4K HDR output with a browser on it. It was always counted and only ever logged at `AVK_DEBUG`, which is filtered, so grepping for the message found nothing and the mechanism was crossed off twice. Nonzero in steady state means uploads are wrapping the ring inside a single frame |
| `shm_staging_returned` | staging buffers given back as their copy was submitted. Staging is scratch space, not state: everything after the submit belongs to the GPU, and the retire queue already knows how to wait for it. One 236-commit run of a client presenting a fresh 56MB buffer every frame used **two** staging buffers in total |
| `shm_staging_reaped` | staging buffers taken back from entries that stopped drawing. Staging used to be released only when its `wl_buffer` was destroyed, which is fine for a client that presents a fresh buffer and drops the old one and useless for one that keeps them — Firefox keeps them. Measured live: 79 cached entries, 68 allocations, `retire_entries_live` at **0**, so the cache had nothing to hand out and every new buffer faulted a fresh 56MB. An entry idle for a second gives its scratch space back, on demand just before a large allocation would otherwise fault |
| `shm_staging_waits` / `shm_staging_wait_us_max` | times the host wanted to write a staging slot the GPU was still reading out of, and the longest such wait. Staging is double-buffered per buffer so this should be **0**: the slot being written is two submissions old. Nonzero means uploads are outrunning the transfer, and the wait is what stands between that and torn pixels — a single staging buffer here produced horizontal banding, because the copy walks staging in row order and picks up whichever generation had reached each row |
| `commit_us_max` / `commit_us_avg` | wall clock inside a surface-commit handler. What a client's commit costs the compositor, as opposed to what the frame costs |
| `handler_us_max` / `handler_us_avg` | wall clock inside an event-loop handler, whatever it was handling. The number that says whether the loop is being blocked at all |
| `handler_over_10ms` / `handler_over_30ms` | handlers that ran past 10ms and past 30ms. At 144Hz the frame budget is 6.9ms, so a single count here is a dropped frame the user saw. **Both should be 0** — thresholds rather than a mean because a stall is an event, and an average over thousands of fast handlers hides it |
