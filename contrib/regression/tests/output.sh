...
# ── THE VRR RATE GATE, AS FAR AS A HEADLESS OUTPUT CAN SEE IT ─────────────
#
# WHAT THIS CANNOT TEST, SAID PLAINLY. The gate only engages once VRR is
...
# here. So the held answer, the cancel, and the presentation-interval
# measurement that releases it are NOT exercised by this suite and have to be
# verified on real hardware. The counters below are how that is done, because
# they are the difference between "no modesets happened" and "no modesets were
# needed".
#
# What this does test is that the counters exist and that an output which never
# had VRR on has never held an answer about turning it off -- the half that
# disappears silently in a refactor.
